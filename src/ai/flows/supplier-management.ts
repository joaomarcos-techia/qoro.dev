...
const updateSupplierFlow = ai.defineFlow(
    {
        name: 'updateSupplierFlow',
        inputSchema: UpdateSupplierSchema.extend(ActorSchema.shape),
        outputSchema: z.object({ id: z.string() })
    },
    async (input) => supplierService.updateSupplier(input.id, input, input.actor)
);
...
export async function updateSupplier(input: z.infer<typeof UpdateSupplierSchema> & z.infer<typeof ActorSchema>): Promise<{ id: string; }> {
    return updateSupplierFlow(input);
}
