...
const statusMap: Record<BillProfile['status'], { text: string; color: string }> = {
    ...
    overdue: { text: 'Vencido', color: 'bg-red-500/20 text-red-300' },
};
...
export function BillTable({ onEdit, refreshKey }: BillTableProps) {
  ...
  const handleDelete = async (billId: string) => {
    ...
    try {
        ...
        setError("Não foi possível excluir a pendência.");
    }
  };
  ...
  const columns: ColumnDef<BillProfile>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            const text = type === 'receivable' ? 'A Receber' : 'A Pagar';
            return (
                ...
                <DropdownMenu>
                 ...
                 <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Esta ação não pode ser desfeita. Isso excluirá permanentemente a pendência.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
  ...
  if (error) {
    return <div className="text-destructive text-center min-h-[400px] p-4 bg-destructive/10 rounded-lg">{error}</div>;
  }
  ...
  if (data.length === 0) {
    return (
        <div className="flex flex-col items-center justify-center text-center min-h-[400px]">
            ...
            <h3 className="text-xl font-bold text-foreground">Nenhuma pendência encontrada</h3>
            <p className="text-muted-foreground mt-2">Adicione suas contas a pagar e receber para vê-las aqui.</p>
        </div>
    )
  }
